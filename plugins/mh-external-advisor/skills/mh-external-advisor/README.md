...
授權層有兩種形態，預設為**逐次授權**：使用者每次點名要問哪一支。授權單位是一個討論——點名一次即授權該討論最多 3 輪，續輪不得換顧問、換主題或擴張外送範圍。

**自主模式**讓主 AI 自行決定何時問、問什麼、叫顧問看哪裡、問幾次，不必逐次點名——由使用者說「啟動外部顧問自主模式」開啟、「停止外部顧問自主模式」關閉，持續到使用者關閉為止。啟動要兩個條件：指令出自使用者本人且在主對話中、指定顧問已確定（限一支）。顧問唯讀，速率上限由節流桶把關。環境隔離不是啟動條件，未聲明時只在啟動確認中警告。啟動後主 AI 才按需載入〈自主模式執行辦法〉（模式契約與生命週期、三個諮詢目的的觸發與問法、單次呼叫協定、回收採信與揭露），未啟動時不讀。

兩種模式一體適用：三層清單、送出前的硬規則、顧問回覆的採信原則（自主模式下顧問輸出只能形成候選與待裁決項）、有分歧時的討論輪次（最少 2 輪最多 3 輪）、回報給使用者的形態（預設只報共識方案與主 AI 的決定）、子 Agent 規則、`allowed-tools`（Bash 仍照常詢問）。自主模式狀態只存在該次對話，不落檔、不跨 session；每次呼叫都對使用者印一行「正嘗試請哪支 AI 做什麼」（顯示寬度 ≤100）。
