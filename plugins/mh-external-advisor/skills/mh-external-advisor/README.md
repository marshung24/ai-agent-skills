...
兩種模式一體適用：三層清單、送出前的硬規則、顧問回覆的採信原則（自主模式下顧問輸出只能形成候選與待裁決項）、子 Agent 規則、`allowed-tools`（Bash 仍照常詢問）。自主模式狀態只存在該次對話，不落檔、不跨 session；每次呼叫都對使用者印一行「正嘗試請哪支 AI 做什麼」（顯示寬度 ≤100）。
