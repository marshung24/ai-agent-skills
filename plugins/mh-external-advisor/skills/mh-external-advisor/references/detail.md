...
> 本檔是**執行期**參考：呼叫、排查、寫諮詢 prompt 時查。
> 腳本內部行為、各家額度介面、底層 CLI 指令與設計取捨見 [docs/internals.md](../docs/internals.md)（維護用）。

## 清單腳本（getter / setter）

顧問清單分三層：**支援**＝`scripts/` 下可執行的 `ask-*.sh`（檔案系統為單一事實來源）；**啟用**＝使用者寫進設定檔的清單；**授權**＝本次對話中被點名的那支，或已有效啟動的自主模式（見 SKILL.md〈授權模式〉）。
...
- **對外一律稱 `session_id`**（參數、警語、輸出行皆同）；各 CLI 內部的真實名稱（codex＝thread_id、agy＝conversation id）只在 [docs/internals.md](../docs/internals.md)〈底層指令〉表出現。
