...
while getopts "r:" opt; do
  case "$opt" in
    ...
    *) echo "用法: $0 [-r <thread_id>] [--] \"<prompt>\"" >&2; exit 2 ;;
  esac
done
shift $((OPTIND - 1))
...
# DECISION: resume 失敗不自動改開新 thread 重送——prompt 可能含有副作用指示，
#           自動重送有重複執行風險；改以 exit 3 讓呼叫端自行決定是否重送
if [ -n "$RESUME" ]; then
  printf '%s' "$PROMPT" | codex exec resume "${COMMON[@]}" "$RESUME" - >"$LOG" 2>"$ERR"
  if [ $? -ne 0 ]; then
    { echo "錯誤：resume 失敗（thread 可能已失效）。確認無重複執行疑慮後，可不帶 -r 重送以開新對話。stderr 末幾行："
      tail -5 "$ERR"; } >&2
    exit 3
  fi
else
...
TID="$(jq -Rrs 'split("\n") | map(fromjson? // empty)
  ...
  | first // empty' "$LOG")"
# resume 時若事件未重印 id，沿用傳入的 RESUME 以維持延續性
[ -n "$RESUME" ] && [ -z "$TID" ] && TID="$RESUME"
...
if [ -n "$TID" ]; then
  ...
  echo "[warn] 未取得 thread_id，本段對話無法延續" >&2
fi
